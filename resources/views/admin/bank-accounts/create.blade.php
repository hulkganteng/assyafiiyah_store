<x-app-layout>
    <div class="py-12 bg-white md:bg-transparent">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6">
                <h1 class="text-3xl font-serif font-bold text-gray-800">Tambah Rekening</h1>
                <p class="text-sm text-gray-500 mt-1">Masukkan detail rekening untuk pembayaran.</p>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-3xl border border-gray-100 p-8">
                <form action="{{ route('admin.bank-accounts.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="bank_name" value="{{ old('bank_name') }}" required class="w-full rounded-xl border-gray-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 bg-gray-50">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Nomor Rekening</label>
                        <input type="text" name="account_number" value="{{ old('account_number') }}" required class="w-full rounded-xl border-gray-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 bg-gray-50">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Atas Nama</label>
                        <input type="text" name="account_name" value="{{ old('account_name') }}" class="w-full rounded-xl border-gray-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 bg-gray-50">
                    </div>
